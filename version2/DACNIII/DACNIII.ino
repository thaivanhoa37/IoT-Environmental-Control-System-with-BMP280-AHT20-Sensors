...
bool device1State = false; // Heater: true = ON, false = OFF
bool device2State = false; // Humidifier: true = ON, false = OFF
...
void publishMQTTDevice() {
  ...
  // Hiển thị trạng thái dựa trên logic đảo của Relay
  bool device1ActualState = !digitalRead(DEVICE1_PIN); // Relay đảo logic: LOW = ON, HIGH = OFF
  bool device2ActualState = !digitalRead(DEVICE2_PIN);
  client.publish(TOPIC_STATE_DEVICE1, device1ActualState ? "ON" : "OFF");
  Serial.print("Đã gửi: "); Serial.print(device1ActualState ? "ON" : "OFF"); Serial.println(" đến topic iot/state/device1");
  
  client.publish(TOPIC_STATE_DEVICE2, device2ActualState ? "ON" : "OFF");
  Serial.print("Đã gửi: "); Serial.print(device2ActualState ? "ON" : "OFF"); Serial.println(" đến topic iot/state/device2");
...
void TaskDisplay(void *pvParameters) {
  while (1) {
    if (!isAPMode && updateDisplay && oledInitialized) {
      ...
      bool device1ActualState = !digitalRead(DEVICE1_PIN); // Relay đảo logic: LOW = ON, HIGH = OFF
      bool device2ActualState = !digitalRead(DEVICE2_PIN);
      display.print("Dev1: "); display.println(device1ActualState ? "ON" : "OFF");
      display.print("Dev2: "); display.println(device2ActualState ? "ON" : "OFF");
  ...
  while (1) {
    ...
    if (!isAutoMode) {
      if (digitalRead(BUTTON_DEVICE1) == LOW && (currentTime - lastDebounceTime) > debounceDelay) {
        ...
        digitalWrite(DEVICE1_PIN, !device1State);
        bool device1ActualState = !digitalRead(DEVICE1_PIN); // Kiểm tra trạng thái thực tế
        Serial.print("Device 1 switched to: "); Serial.println(device1ActualState ? "ON" : "OFF");
        updateDisplay = true;
        publishDeviceFlag = true;
        lastDebounceTime = currentTime;
      }
      ...
      if (digitalRead(BUTTON_DEVICE2) == LOW && (currentTime - lastDebounceTime) > debounceDelay) {
        ...
        digitalWrite(DEVICE2_PIN, !device2State);
        bool device2ActualState = !digitalRead(DEVICE2_PIN); // Kiểm tra trạng thái thực tế
        Serial.print("Device 2 switched to: "); Serial.println(device2ActualState ? "ON" : "OFF");
        updateDisplay = true;
        publishDeviceFlag = true;
        lastDebounceTime = currentTime;
      }
    ...
    if (isAutoMode && dataUpdated) {
      ...
      if (prevDevice1 != device1State || prevDevice2 != device2State) {
        ...
        Serial.print("Device 1: "); Serial.print(device1ActualState ? "ON" : "OFF");
        Serial.print(", Device 2: "); Serial.println(device2ActualState ? "ON" : "OFF");
        publishDeviceFlag = true;
        updateDisplay = true;
      }
      dataUpdated = false;
