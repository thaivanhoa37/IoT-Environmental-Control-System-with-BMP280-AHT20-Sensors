...
const char* TOPIC_STATE_ALTITUDE = "node2/state/altitude";
...
const char* TOPIC_STATE_PRESSURE_THRESHOLD = "node2/state/pressure_threshold";
...
const char* TOPIC_CONTROL_PRESSURE_THRESHOLD = "node2/control/pressure_threshold";
...
float pressureThreshold = 1013.25; // hPa
...
void TaskAPTimeout(void *pvParameters) {
  const unsigned long AP_TIMEOUT = 15000; // 15 giây
  ...
  if (xSemaphoreTake(mqttMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
    ...
    while (!client.connected() && retryCount < 3) {
      ...
      if (client.connect(mqtt_client_id)) {
        ...
        client.subscribe(TOPIC_CONTROL_PRESSURE_THRESHOLD);
        failedReconnectAttempts = 0;
        xSemaphoreGive(mqttMutex);
        return true;
      } else {
...
void publishMQTTDevice() {
  if (xSemaphoreTake(mqttMutex, pdMS_TO_TICKS(1000)) == pdTRUE) {
    ...
    snprintf(msg, 50, "%.1f", pressureThreshold);
    client.publish(TOPIC_STATE_PRESSURE_THRESHOLD, msg);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (topicStr == TOPIC_CONTROL_MODE) {
    ...
    dataUpdated = true;
  } else if (topicStr == TOPIC_CONTROL_PRESSURE_THRESHOLD) {
    pressureThreshold = messageTemp.toFloat();
    Serial.print("Pressure Threshold updated via MQTT to: "); Serial.println(pressureThreshold);
    thresholdUpdated = true;
    dataUpdated = true;
  }
}
...
void TaskReadSensor(void *pvParameters) {
  while (1) {
    ...
    if (isnan(temperature) || isnan(humidity) || isnan(pressure)) {
      Serial.println("❌ Lỗi đọc cảm biến AHT20 hoặc BMP280!");
    } else {
      tempSum += temperature;
      humSum += humidity;
      pressSum += pressure;
...
void TaskPrintData(void *pvParameters) {
  while (1) {
    if (countTime >= 15) { // Trung bình mỗi 30 giây (15 lần x 2 giây)
      if (count > 0) {
        avgTemperature = tempSum / count;
        avgHumidity = humSum / count;
        avgPressure = pressSum / count;
        avgAltitude = bmp.readAltitude(1013.25);

        Serial.println("--------------");
        Serial.println("Trung bình 30s:");
        Serial.print("Nhiệt độ: "); Serial.print(avgTemperature); Serial.println("°C");
        Serial.print("Độ ẩm: "); Serial.print(avgHumidity); Serial.println("%");
        Serial.print("Áp suất: "); Serial.print(avgPressure); Serial.println(" hPa");
        Serial.print("Độ cao: "); Serial.print(avgAltitude); Serial.println(" m");
        Serial.println("--------------");
